...
void kernel_base(complex_t *x, double *y, int n) {
    for (int i = 0; i < n; i++) {
...
double perf_test(comp_func f, string desc, int flops) {
    ...
    complex_t *x;
    double    *y, *y_bk;
    build((double**) &x, 2*n, 1);
    build((double**) &y, 1*n, 1);
    build((double**) &y_bk, 1*n, 1);
    rands(y, n, 1, 0.0, 0.999);
    copy_n(y, n, y_bk);      
...
int main(int argc, char **argv) {
  ...
  int n = 1024;
  complex_t *x;
  double *y, *y_base, *y_copy;
  build((double**) &x, 2*n, 1);
  build((double**) &y, 1*n, 1);
  build((double**) &y_base, 1*n, 1);
  build((double**) &y_copy, 1*n, 1);
  rands(y, n, 1, 0.0, 0.999);
  copy_n(y, n, y_base);
  copy_n(y, n, y_copy);
  ...
  destroy((double*) x);
  destroy((double*) y);
  destroy((double*) y_base);
  destroy((double*) y_copy);
